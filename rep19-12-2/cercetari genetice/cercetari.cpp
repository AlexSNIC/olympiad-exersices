...
int findDegree(string Monea, string otherCat){
  ...
  for(char c : Monea){
    auto iter = find(otherCat.begin(), otherCat.end(), c);
    bool hasValue = iter == otherCat.end()? false : true;

    if(hasValue){
      sum++;
      // Decomentează dacă condiția cere ca literele să nu se repete.
      // while(iter != otherCat.end()){
      // auto iter = find(otherCat.begin(), otherCat.end(), c);
         otherCat.erase(iter);
      // }
    }
...
int main(){
  ifstream fin("cercetari.in");
  ofstream fout("cercetari.out");
